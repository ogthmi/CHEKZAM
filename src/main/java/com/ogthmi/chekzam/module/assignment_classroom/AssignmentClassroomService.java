...
public class AssignmentClassroomService {
    ...
    public Page<AttachedAssignmentDTO> getAllAssignmentInClassroom(
        ...
        if (sortBy.equals("assignmentName")) {
            sort = Sort.by(Sort.Direction.fromString(direction), "assignmentEntity.assignmentName");
        } else if (sortBy.equals("assignmentId")) {
            sort = Sort.by(Sort.Direction.fromString(direction), "assignmentEntity.assignmentId");
        } else {
    ...
    @Transactional
    public void updateAssignmentInfo(String assignmentId, AttachedAssignmentDTO attachedAssignmentDTO) {
        String assignmentName = attachedAssignmentDTO.getAssignmentName();
        if (assignmentName == null || assignmentName.isBlank()) {
            throw new ApplicationException(ExceptionMessageCode.ASSIGNMENT_NAME_EMPTY);
        }
        AssignmentEntity assignmentEntity = assignmentService.findAssignmentById(assignmentId);
        assignmentEntity.setAssignmentName(attachedAssignmentDTO.getAssignmentName());
        assignmentEntity.setDescription(attachedAssignmentDTO.getDescription());
        assignmentEntity.setAssignmentType(attachedAssignmentDTO.getAssignmentType());
        assignmentRepository.save(assignmentEntity);
        if (attachedAssignmentDTO.getClassroomId() != null){
            updateAttachedAssignmentInfoInClassroom(assignmentEntity, attachedAssignmentDTO);
        }
        log.info("Cập nhật bài tập {} thành công", assignmentId);
    }

    public void updateAttachedAssignmentInfoInClassroom(AssignmentEntity assignment, AttachedAssignmentDTO attachedAssignmentDTO) {
        AssignmentClassroomEntity assignmentClassroomEntity = findAssignmentClassroomEntity(assignment.getAssignmentId(), attachedAssignmentDTO.getClassroomId());
