...
public class AssignmentService {
    ...
    public boolean hasAccessToAssignment(UserEntity user, AssignmentEntity assignment) {
        ...
        if (roles.contains(Role.STUDENT)) {
            return user.getClassroomList().stream()
                    .anyMatch(classroomStudentEntity -> {
                        return classroomStudentEntity.getClassroomEntity().getAssignmentList().stream()
                                .anyMatch(assignmentClassroomEntity ->
                                        assignmentClassroomEntity.getAssignmentEntity().equals(assignment)
                                );
                    });
        }
        return false;
    ...
    private void validateAttachAssignmentRequest(AssignmentRequest assignmentRequest) {
        ...
        for (int questionIndex = 0; questionIndex < questionList.size(); questionIndex++) {
            boolean hasCorrectAnswer = isHavingCorrectAnswer(questionList, questionIndex);
            if (!hasCorrectAnswer) {
    ...
    private static boolean isHavingCorrectAnswer(List<QuestionDTO> questionList, int questionIndex) {
        List<AnswerDTO> answerList = getAnswerDTOList(questionList, questionIndex);
    ...
    private static List<AnswerDTO> getAnswerDTOList(List<QuestionDTO> questionList, int questionIndex) {
