...
import {CustomerFormTextArea} from "../../components/common/CustomFormTextArea";
...
export const CreateAssignmentPage = () => {
    ...
    const handleSubmit = async () => {
        ...
        // 3. Format thời gian trước khi submit
        const payload = {
            ...assignment,
            startTime: assignment.startTime ? moment(assignment.startTime).format('DD-MM-YYYY HH:mm') : null,
            endTime: assignment.endTime ? moment(assignment.endTime).format('DD-MM-YYYY HH:mm') : null
        };

        console.log('Submitted data:', payload); // Log ra payload đã format

        try {
            const { success, data, message } = await createData(EntityTypes.assignment.DETAILS, null, null, payload);
            if (success) {
    ...
    return (<Container className="p-2 mt-3">
        <Row className="my-3">
            <div className={"p-md-0 mx-md-0 mx-3 d-md-flex justify-content-between"}>
                ...
                {isModalOpen && <FileUploadModal modalTitle={"Đọc bài tập từ file"}
                                                 show={isModalOpen}
                    ...
                    <Row>
                            ...
                            <Form.Group className="mb-3">
                                ...
                                <Form.Control
                                    ...
                                    onChange={(e) => setAssignment({
                                        ...assignment, duration: parseInt(e.target.value),
                                    })}
                                />
                            </Form.Group>
                            <Form.Group className="mb-3">
                                <Form.Label>Số lần làm bài tối đa</Form.Label>
                                <Form.Control
                                    type="number"
                                    placeholder={'Nhập 0 nếu không giới hạn'}
                                    value={assignment.maxAttempts}
                                    onChange={(e) => setAssignment({
                                        ...assignment, maxAttempts: parseInt(e.target.value),
                                    })}
                                />
                            </Form.Group>
                        </Col>
                        ...
                        <Col md={4}>
                            <Form.Group className="mb-3">
                                ...
                                <ReactDatetime
                                    value={assignment.startTime || null} // Cho phép null nếu không có thời gian
                                    onChange={(date) => setAssignment({
                                        ...assignment,
                                    ...
                                    timeFormat="HH:mm"
                                    inputProps={{
                                        placeholder: 'Để trống nếu không thiết lập',
                                    }}
                                />
                            </Form.Group>
                            ...
                            <Form.Group className="mb-3">
                                ...
                                <ReactDatetime
                                    value={assignment.endTime || null}
                                    onChange={(date) => setAssignment({...assignment, endTime: date ? date : null})}
                                    dateFormat="DD-MM-YYYY"
                                    timeFormat="HH:mm"
                                    inputProps={{
